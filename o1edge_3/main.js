...
(function () {
    ...
    function updateCursor(dX) {
        ...
        // Feed telemetry to Ising-Worker
        if (npuActive && isingWorker) {
            isingWorker.postMessage({ type: 'COMPUTE', driftX: n, driftY: Math.random() * 0.1 });
        }
    }

    // =========================================================================
    // 9. IGNITE + FULLSCREEN BUTTONS + NPU WORKER
    // =========================================================================
    let isingWorker = null;
    let npuActive = false;
    let npuChaos = 0.0;
    ...
    document.getElementById('btn-ignite').addEventListener('click', () => {
        ...
        // Start Ising Worker
        try {
            isingWorker = new Worker('isingWorker.js');
            isingWorker.onmessage = (e) => {
                if (e.data.type === 'INIT_OK') {
                    npuActive = true;
                    console.log("[IsingWorker] Booted using backend:", e.data.backend);
                    const npuContainer = document.querySelector('.npu-inactive');
                    if (npuContainer) {
                        npuContainer.classList.remove('npu-inactive');
                        npuContainer.classList.add('npu-active');
                        npuContainer.querySelector('span').textContent = 'NPU: ' + e.data.backend;
                    }
                } else if (e.data.type === 'RESULT') {
                    npuChaos = e.data.chaos;
                }
            };
            isingWorker.postMessage({ type: 'INIT' });
        } catch(err) {
            console.error('[HAUS] Worker failed to boot', err);
        }
    ...
    function renderLoop(now) {
        ...
        if (gl && prog) {
            // Mix chaos into the drift visual
            const visualDrift = smoothDrift + (npuChaos * 0.2);
            
            // WebGL2 Feynman Foam
            gl.useProgram(prog);
            gl.uniform2f(uRes,   canvas.width, canvas.height);
            gl.uniform1f(uTime,  t);
            gl.uniform1f(uDrift, visualDrift);
